...
<body>
	                ...
	                <div class="board-content">
	                  <form action="<%= request.getContextPath() %>/onstudy/searchBoard?oNo=<%= oNo %>" method="post" class="board-form" onsubmit="return checkSearchVal();"> 
	                    <div id="search-wrap">                  
	                      ...
	                      <div class="col-md-12">
	                        <div class="row"> 
	                          <table class="board onstudy-board">
	                            <thead>
	                              <tr>
	                                <th width="5%">No</th>
	                                <th width="40%">제목</th>
	                              ...
	                              <% }else{ %>
	                              	<% for(CBoard board : bList){%>
	                              		<tr>
	                              			...
	                              			<td class="has-list"><%= board.getRowNum()%></td>
	                              			<td class="has-list"><%= board.getBoardTitle()%></td>
	                              			<td class="has-list"><%= board.getBoardWriterId()%></td>
	                              			<td class="has-list"><%= board.getBoardCount()%></td>
	                          ...
	                          <%} else{%>
	                          	<%if(todayCount > 0){ %>
	                          	<a class="write-content-btn form-control gray-btn" style="width:10%" disabled>글쓰기</a>
								<p align="right">오늘 인증이 완료되어 글작성이 제한됩니다.</p>
								<%}else{ %>
	                          	<a href="<%= request.getContextPath() %>/onstudy/insertForm?oNo=<%= oNo %>" class="write-content-btn form-control orange-hover-btn" style="width:10%">글쓰기</a>
								<%} %>
	                          <%} %>
	                        </div>
	                      </div>
	                      <ul class="pagination">
	                      	<% if(currentPage>1){%>
	                        <li class="page-item">
	                          <a class="page-link" href="<%= request.getContextPath() %>/onstudy/boardList?oNo=<%= oNo %>&currentPage=1">&lt;&lt;</a>
	                        </li>
	                        <li class="page-item">
	                          <a class="page-link" href="<%= request.getContextPath() %>/onstudy/boardList?oNo=<%= oNo %>&currentPage=<%= currentPage-1 %>">&lt;</a>
	                        </li>
	                        <% } %>
	                        ...
	                        <% for(int p = startPage; p <= endPage; p++){ %>
			                    ...
			                    <% }else{%>
			                    	<li class="page-item">
			                          <a class="page-link" href="<%= request.getContextPath() %>/onstudy/boardList?oNo=<%= oNo %>&currentPage=<%= p %>"><%= p %></a>
			                        </li>
	                        	<% }%>
                        	<% }%>
	                        ...
	                        <li class="page-item">
	                          <a class="page-link" href="<%= request.getContextPath() %>/onstudy/boardList?oNo=<%= oNo %>&currentPage=<%= maxPage %>">&gt;&gt;</a>
	                        </li>
	                        <% } %>
	                      </ul>
	                   </form>
	...
	<script>
	        	$(function(){
		        	...
		        	/* 게시글 상세 조회용  */
	        		$(".onstudy-board .has-list").on({
	        			click : function(){
	        				var boardNo = $(this).parent().children().eq(0).text();
	        				location.href="<%= request.getContextPath() %>/onstudy/boardDetail?oNo="+<%=oNo%>+"&bNo="+boardNo+"&currentPage="+<%=currentPage%>;
	        			}
	        		});
	        	...
	        	// 검색내용없이 검색했을때 다시 첫 페이지로 이동
 	        	function checkSearchVal(){
	        		...
	        		if($("#searchValue").val().length==0){
	        			location.href="boardList?oNo="+<%= oNo %>;
		        		return false;
	        		}
					return true;	        		
	        	} 
