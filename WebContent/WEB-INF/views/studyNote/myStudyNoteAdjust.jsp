...
<body>
	...
	<div id="container"
		...
		<form
			action="<%=request.getContextPath()%>/StudyNoteController/updateStudyNote"
			method="post" role="form">
		<div class="row wty-123">
			<div class="col-2"></div>
			<div class="col-3">

				<b style="font-size: 20px;">학습노트 수정</b>
			</div>
				...
				<div class="row">
					...
					<div class="col-6">
						...
						<input type="text" class="col-12" id="input" name="title"
							placeholder="제목 입력 칸" style="size: 12px;" value="<%= title%>">
					<input type="hidden" name="SNnum" value="<%=SNnum%>">
					
					</div>
					<div class="col-3">
						<button class="col-5 pointer " id="search-input"
							style="font-size: 20px; border: none; background-color: #002d4c; color: white;"
...
<%if(snlist.get(0).getcategoryNM() != null){%>
				...
				var categoryNM = "<%=snlist.get(0).getcategoryNM().trim()%>"
				
				var targetNM =  "#"+categoryNM+"1";
				
				console.log(targetNM);
				
		setTimeout(function() {
	  console.log('Works!');
	  $(targetNM).trigger("click");
...
<%}%>
            ...
            $(function() {
                ...
                var rownum=$(".box-delete").html();
                
                console.log($(".box-delete").html());
                
                console.log("SNnum ="+<%=SNnum%>+"이다");
                var deleteTrigger = $(this).parent().parent().parent()
                $.ajax({
                	
                	url:"<%= request.getContextPath()%>/StudyNoteController/deleteDetail",
                	data: {rownum:rownum,
                		SNnum : <%=SNnum%>	
                	},
                	type : "post",
					...
					<div class="row box-deleted">
						<div class="col-1">
							<div class="rounded box-delete"
								style="width: 100%; margin-top: 40%; font-size: 100%;" ><%=i+1%></div>
						</div>
						<div class="col-11">
							<div class="row">
								...
								<div class="col-5">
									<textarea class="jumbotron" name="setWord" id="" cols="30"
										rows="8"
										style="font-size: 15px; width: 100%; size: 100%; padding: 0%;">
<%= snslist.get(i).getSetWord().trim() %></textarea>
								</div>
								<div class="col-1"></div>
								<div class="col-5">
									<textarea class="jumbotron" name="setMean" id="" cols="30"
										rows="8"
										style="font-size: 15px; width: 100%; size: 100%; padding: 0%;">
<%= snslist.get(i).getSetMean().trim()%></textarea>
								</div>
							</div>
			...
			<div class="container" id="word-collect">
				<div class="container">
					<div class="row box-deleted">
						<div class="col-1">
							<div class="rounded box-delete"
								style="width: 100%; margin-top: 40%; font-size: 100%;" ><%=++i%></div>
						</div>
						<div class="col-11">
							<div class="row">
			...
			<div class="container">
				<div class="row">
					<div style="text-align: center; margin: auto; font-size: 20px;">
						...
						<span>추가하기</span> 
						<input class="add-many" type="text" placeholder="5" style="border: none; width: 30px;" value="5">
					</div>
				</div>
	...
	<script>
        ...
        $(function () {
            ...
            $(document).on("click", ".add-btn", function () {
                ...
                // for (int i=1 ; i<= many ; i++){}  jsp로 추가하여 반복실행 할 것

                
                if (many != null|| many!=0) {
                    for (var j = 0; j < many; j++) {

                        $("#word-collect").append('<div class="container">'
    		                    + '<div class="row ">'
    		                    + '<div class="col-1" >'
    		                    + '<div class="rounded box-delete " style="width: 100%;  margin-top: 40%; font-size: 100%;">'+(k+1)+'</div>'
    		                    + '</div>'
    		                    + '<div class="col-11">'
    		                    + ' <div class="row">'
					...
					$("#word-collect").append('<div class="container">'
		                    + '<div class="row ">'
		                    + '<div class="col-1" >'
		                    + '<div class="rounded box-delete " style="width: 100%;  margin-top: 40%; font-size: 100%;">'+k+'</div>'
		                    + '</div>'
		                    + '<div class="col-11">'
		                    + ' <div class="row">'
